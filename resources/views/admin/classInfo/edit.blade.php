...
@section('body')
 ...
 <div class="card">
            ...
            <!-- /.card-header -->
            <div class="card-body">
              <form action="{{ route('admin.classinfo.update') }}" id="form_validation" method="POST">
              	@csrf
              	<div class="form-group">
                        <label>Department_Name</label>
                        <select class="form-control" name="section_id">
                          @foreach($departments as $cat)
                          <option value="{{ $cat->id }}" {{ $cats->section_id == $cat->id ? 'selected' : '' }}>{{ $cat->department_name }}</option>
                          @endforeach
                      </select>
                      </div>
              	<div class="form-group">
                    <label for="exampleInputEmail1">Section Name</label>
                    <input type="text" class="form-control" name="section_name" value="{{ $cats->section_name }}" id="exampleInputEmail1" placeholder="Enter Name">
