...
@section('body')
 <div class="content-wrapper">
    ...
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-2">
            <a href="{{ route('admin.subject.create') }}" type="button" class="btn btn-block bg-navy color-palette text-light">Add</a>
          </div>
          <div class="col-sm-6">
 ...
 <div class="card">
            <div class="card-header">
              <h3 class="card-title">Suject List</h3>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
             ...
             @foreach($categories as $key=>$category)
                <tr>
                  ...
                  <td>
                    @foreach($sems as $sem)
                    @if($sem->id == $category->semester_id)
                    {{ $sem->semestername }} 
                    @endif
                    @endforeach
                  </td>
                   <td>
                    @foreach($deps as $sem)
                    @if($sem->id == $category->department_id)
                    {{ $sem->department_name }} 
                    @endif
                    @endforeach
                  </td>
                  <td>{{ $category->subject_name }} </td>
                  <td>
                  ...
                  <td><button  type="button" class="btn btn-danger text-light" onclick="deleteTag({{ $category->id }})"><i class="fas fa-trash-alt"></i></button>
                    <form id="delete-form-{{ $category->id }}" action="{{ route('admin.subject.destroy',$category->id) }}" method="POST" style="display: none;">
                                                    @csrf
